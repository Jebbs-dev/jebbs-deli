...
import { useFetchVendorById } from "@/modules/shop/queries/fetch-vendor-by-id";
import { ArrowLeft } from "lucide-react";
import { useFetchProductsByVendor } from "@/modules/shop/queries/fetch-products-by-vendor";
...
const VendorPage = () => {
  ...
  const { data: vendor, isPending } = useFetchVendorById(String(vendorId));
  // const { data: products, isPending } = useFetchProductsByVendor(String(vendorId));
  ...
  const handleAddToCart = (product: Product) => {
    // Ensure vendor information is attached to the product
    const productWithVendor = {
      ...product,
      vendor: vendor, // Attach the full vendor object
      vendorId: vendor?.id, // Also attach vendorId for direct access
    };

    addItem(productWithVendor);
    toast({
    ...
    <div className="pt-[40px] md:pt-[81px] h-full">
      <div className="md:container flex flex-row mx-auto px-10 2xl:px-0">
        ...
        <div className="w-[37%] border-l-2 px-4 hidden md:block overflow-scroll h-screen">
          <div className="mb-10">
            <h1 className="text-xl">Orders from {vendor?.name}</h1>
          </div>
